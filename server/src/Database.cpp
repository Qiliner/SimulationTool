...
bool Database::createTaskTables() {
    const std::string createTasksTable = R"(
    ...
    return executeSql(createTasksTable);
}
...
std::vector<std::tuple<int, std::string, std::string, int, std::string, int>> Database::getAllTasks(int userId) {
    std::lock_guard<std::mutex> lock(m_mutex);
    std::vector<std::tuple<int, std::string, std::string, int, std::string, int>> result;
    std::string sql;
    if (userId == -1) {
        sql = "SELECT id, name, description, owner_id, content, updated_at FROM tasks ORDER BY updated_at DESC";
    }
    else {
        sql = "SELECT id, name, description, owner_id, content, updated_at FROM tasks WHERE owner_id = ? ORDER BY updated_at DESC";
    }
    sqlite3_stmt* stmt = nullptr;
    int rc = sqlite3_prepare_v2(m_db, sql.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) return result;
    if (userId != -1) sqlite3_bind_int(stmt, 1, userId);
    while (sqlite3_step(stmt) == SQLITE_ROW) {
        ...
        result.emplace_back(id, name, desc, ownerId, content, updatedAt);
    }
    sqlite3_finalize(stmt);
    return result;
}

std::tuple<bool, std::string> Database::getTaskContent(int taskId) {
    std::lock_guard<std::mutex> lock(m_mutex);
    const std::string sql = "SELECT content FROM tasks WHERE id = ?";
    sqlite3_stmt* stmt = nullptr;
    int rc = sqlite3_prepare_v2(m_db, sql.c_str(), -1, &stmt, nullptr);
    if (rc != SQLITE_OK) return { false, "" };
    sqlite3_bind_int(stmt, 1, taskId);
...
int Database::getTaskIdByName(const std::string& name) {
    std::lock_guard<std::mutex> lock(m_mutex);
    const std::string sql = "SELECT id FROM tasks WHERE name = ?";
    sqlite3_stmt* stmt = nullptr;
    int rc = sqlite3_prepare_v2(m_db, sql.c_str(), -1, &stmt, nullptr);
